classdef Interface < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        updateSolidsButton            matlab.ui.control.Button
        updateDrawsCheckBox           matlab.ui.control.CheckBox
        ...
        OnupdateDrawsPanel            matlab.ui.container.Panel
    ...
    methods (Access = public)
        ...
        function results = updateFcn(app)
            global control
            app.OnupdateDrawsPanel.Enable='on';
            if control.update_draws_automatic==true
                app.updateDrawsCheckBox.Value=1;
            else
                app.updateDrawsCheckBox.Value=0;
            end
            ...
            if control.direct_dynamics_problem_NE_exported || control.direct_dynamics_problem_VP_exported
                ...
                app.DynamicSolverDropDown.Value = control.dyn_equations_type;
            else
    ...
    methods (Access = public)
        ...
        function results = newVariableSlider(app,name,variable_limits)
            ...
            app.VariableSlidersLabels(n_VariableSliders).Text = name(2);
            
            if n_VariableSliders==1
            app.VariableSlidersLabels(n_VariableSliders).Position = [300,20,6*(length(app.VariableSlidersLabels(n_VariableSliders).Text))-2,22];
            else
            ...
            if n_VariableSliders==1
            app.VariableSliders(n_VariableSliders).Position = [200,30,83,3];
            else
            ...
            value=getValue(name(1));
            if value < app.VariableSliders(n_VariableSliders).Limits(1)
                app.VariableSliders(n_VariableSliders).Limits(1)=value;
            elseif value > app.VariableSliders(n_VariableSliders).Limits(2)
                app.VariableSliders(n_VariableSliders).Limits(2)=value;
            end
            app.VariableSliders(n_VariableSliders).Value=value;
            app.VariableSliders(n_VariableSliders).UserData=[name];
        end
    ...
    methods (Access = private)
        ...
        function DynamicSolverDropDownValueChanged(app, event)
            global control dyn_equations_type
            control.dyn_equations_type=app.DynamicSolverDropDown.Value;
            dyn_equations_type=app.DynamicSolverDropDown.Value;
        end
        ...
        % Button pushed function: AssemblyProblemButton
        function AssemblyProblemButtonPushed(app, event)
            assembly_problem;
            
        end

        % Button pushed function: DirectDynamicsProblemButton
        function DirectDynamicsProblemButtonPushed(app, event)
            direct_dynamics_problem;
        end

        % Button pushed function: InitialAssemblyProblemButton
        function InitialAssemblyProblemButtonPushed(app, event)
            initial_assembly_problem;
        end

        % Button pushed function: PerturbationProblemButton
        function PerturbationProblemButtonPushed(app, event)
            perturbed_dynamic_state_problem;
        end

        % Button pushed function: EigenProblemButton
        function EigenProblemButtonPushed(app, event)
            global dyn_equations_type t_value q n_q q_value dq_value ddq_value epsilon_value lambda_value param_value
            eigenproblem;
        end

        % Button pushed function: EquilibriumProblemButton
        function EquilibriumProblemButtonPushed(app, event)
            global dyn_equations_type t_value q_value  n_q dq_value ddq_value epsilon_value lambda_value n_lambda param_value
            global Dyn_Eq_tol Dyn_Eq_relax updateDraws_automatic
            global has_my_piezewise_functions_in_Phi;
            dynamic_equilibrium_problem;
        end

        % Value changed function: updateDrawsCheckBox
        function updateDrawsCheckBoxValueChanged(app, event)
            global control
            value = app.updateDrawsCheckBox.Value;
            control.update_draws_automatic=value;
        end

        % Value changed function: AssemblyProblemCheckBox
        function AssemblyProblemCheckBoxValueChanged(app, event)
            global control
            value = app.AssemblyProblemCheckBox.Value;
            control.assembly_problem_automatic=value;
        end

        % Value changed function: DirectDynamicProblemCheckBox
        function DirectDynamicProblemCheckBoxValueChanged(app, event)
            global control
            value = app.DirectDynamicProblemCheckBox.Value;
            control.direct_dynamics_problem_automatic=value;
        end

        % Close request function: InterfaceUIFigure
        function InterfaceUIFigureCloseRequest(app, event)
            delete(app)
        end
        
        % Button pushed function: updateSolidsButton
        function updateSolidsButtonPushed(app, event)
            updateSolids;
        end
    end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.InterfaceUIFigure.Position = [100 100 391 480];
            app.InterfaceUIFigure.Name = 'Interface';
            app.InterfaceUIFigure.CloseRequestFcn = createCallbackFcn(app, @InterfaceUIFigureCloseRequest, true);
            app.InterfaceUIFigure.Scrollable = 'on';
            app.InterfaceUIFigure.HandleVisibility = 'on';

            % Create OnupdateDrawsPanel
            app.OnupdateDrawsPanel = uipanel(app.InterfaceUIFigure);
            app.OnupdateDrawsPanel.Title = 'On updateDraws';
            app.OnupdateDrawsPanel.Position = [3 230 170 66];

            % Create AssemblyProblemCheckBox
            app.AssemblyProblemCheckBox = uicheckbox(app.OnupdateDrawsPanel);
            app.AssemblyProblemCheckBox.ValueChangedFcn = createCallbackFcn(app, @AssemblyProblemCheckBoxValueChanged, true);
            ...
            app.DirectDynamicProblemCheckBox = uicheckbox(app.OnupdateDrawsPanel);
            app.DirectDynamicProblemCheckBox.ValueChangedFcn = createCallbackFcn(app, @DirectDynamicProblemCheckBoxValueChanged, true);
            ...
            app.DynamicSolverDropDownLabel.Position = [6 26 90 22];
            ...
            app.DynamicSolverDropDown.Items = {'Newton-Euler', 'Virtual_Power', 'Undefined'};
            app.DynamicSolverDropDown.ValueChangedFcn = createCallbackFcn(app, @DynamicSolverDropDownValueChanged, true);
            app.DynamicSolverDropDown.Position = [5 5 169 22];
            app.DynamicSolverDropDown.Value = 'Undefined';

            % Create SolvePanel
            app.SolvePanel = uipanel(app.InterfaceUIFigure);
            app.SolvePanel.Title = 'Solve';
            app.SolvePanel.Position = [3 62 170 154];

            % Create AssemblyProblemButton
            app.AssemblyProblemButton = uibutton(app.SolvePanel, 'push');
            app.AssemblyProblemButton.ButtonPushedFcn = createCallbackFcn(app, @AssemblyProblemButtonPushed, true);
            app.AssemblyProblemButton.HorizontalAlignment = 'left';
            ...
            app.InitialAssemblyProblemButton.ButtonPushedFcn = createCallbackFcn(app, @InitialAssemblyProblemButtonPushed, true);
            app.InitialAssemblyProblemButton.HorizontalAlignment = 'left';
            ...
            app.DirectDynamicsProblemButton.ButtonPushedFcn = createCallbackFcn(app, @DirectDynamicsProblemButtonPushed, true);
            app.DirectDynamicsProblemButton.HorizontalAlignment = 'left';
            app.DirectDynamicsProblemButton.Position = [1 70 151 22];
            app.DirectDynamicsProblemButton.Text = 'Direct Dynamic Problem';

            % Create EquilibriumProblemButton
            app.EquilibriumProblemButton = uibutton(app.SolvePanel, 'push');
            app.EquilibriumProblemButton.ButtonPushedFcn = createCallbackFcn(app, @EquilibriumProblemButtonPushed, true);
            app.EquilibriumProblemButton.HorizontalAlignment = 'left';
            ...
            app.EigenProblemButton.ButtonPushedFcn = createCallbackFcn(app, @EigenProblemButtonPushed, true);
            app.EigenProblemButton.HorizontalAlignment = 'left';
            ...
            app.PerturbationProblemButton.ButtonPushedFcn = createCallbackFcn(app, @PerturbationProblemButtonPushed, true);
            app.PerturbationProblemButton.HorizontalAlignment = 'left';
            app.PerturbationProblemButton.Position = [1 7 129 22];
            app.PerturbationProblemButton.Text = 'Perturbation Problem';

            % Create updateDrawsCheckBox
            app.updateDrawsCheckBox = uicheckbox(app.InterfaceUIFigure);
            app.updateDrawsCheckBox.ValueChangedFcn = createCallbackFcn(app, @updateDrawsCheckBoxValueChanged, true);
            app.updateDrawsCheckBox.Text = 'updateDraws';
            app.updateDrawsCheckBox.Position = [4 298 93 22];

            % Create updateSolidsButton
            app.updateSolidsButton = uibutton(app.InterfaceUIFigure, 'push');
            app.updateSolidsButton.ButtonPushedFcn = createCallbackFcn(app, @updateSolidsButtonPushed, true);
            app.updateSolidsButton.HorizontalAlignment = 'left';
            app.updateSolidsButton.Position = [3 334 100 23];
            app.updateSolidsButton.Text = 'updateSolids';
