...
%                        obj.Name=name;
%                     else
%                         error('Vector3D(name,value,base_name): name must be a string with a valid variable name')
%                     end
%                 end
%                 value=varargin{2};
%                 base_name=varargin{3};
            end
            if nargin > 0
                if isnumeric(value) || isa(value,'sym')
                    if (size(value,1)==3 && size(value,2)==1) || (size(value,2)==3)
                        obj.Value = sym(value);
                    elseif (size(value,1)==1 && size(value,2)==3)
                        obj.Value = sym(value)';
                    else
                        error('value must be 3x1, 1x3 or 3x3 numeric or symbolic array');
                    end
                else
                    error('value must be 3x1 , 1x3 or 3x3  numeric or symbolic array');
                end
                if not(isempty(base_name))
                    if ischar(base_name)
                        obj.Base = BasesNames(base_name);
                    elseif isa(base_name,'double') && isscalar(base_name) && base_name >= 0 && base_name <= length(Bases);
                        obj.Base = base_name;
                    else
                        error('base_name must be a string or a integer number 0:num_Bases')
                    end
                end
            end
        end
        ...
        %-------------------------------------------
        function r = Components_inBase(o1,base)
            global BasesNames
            if isempty(o1.Value)
                r = Vector3D();
            else
        ...
        %-------------------------------------------
        function subs_v=subs(v, match, replacement)  
            subs_v=Vector3D(subs(v.Value, match, replacement),v.Base);
        ...
        %-------------------------------------------
        function versor=versor(v)
            versor=1/norm(v.Value)*v;
        end
        %-------------------------------------------
        function versor=unit(v)
            versor=1/norm(v.Value)*v;
        end
        %-------------------------------------------
        ...
        function disp(obj)
            ...
            elseif isempty(obj.Base)
                ...
                disp(['Vector3D in base: ','''',char(NamesBases(obj.Base).Base_name),'''']);
            end
        end
        %-------------------------------------------
